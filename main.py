...
def svm_predict(th2,rects,boxes):
	## HOG+SVM prediction
	svm = cv2.ml.SVM_load(r"F:\Python\circuit_recognizer-master\svm_data.dat")    #Change Path
	hog = get_hog()
...
if __name__ == "__main__":
	...
	cv2.moveWindow("recognizer",400,10)
	cv2.setMouseCallback('recognizer',mouse_event)

	src = cv2.imread(r"F:\Python\circuit_recognizer-master\Circuit 4.jpeg")    # Change Path
	...
	for i in range(len(ends[0])):
		image = cv2.circle(image, (ends[1][i], ends[0][i]), 1, (0,0,255), -1)
